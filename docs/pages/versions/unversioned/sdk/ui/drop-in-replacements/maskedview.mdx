...
Under the hood this component bridges arbitrary React Native children into the platform-specific `@expo/ui` masking primitives via `RNHostView`:
...
Pass a `LinearGradient` from `expo-linear-gradient` (or any view that paints with varying alpha) as the `maskElement` to fade out content along an axis.
